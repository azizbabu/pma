...
<div class="table-responsive">
    <table class="table table-hover table-striped table-vertical-middle">
        ...
        <tbody>
            <tr>
                <td>
                    {!! Form::select('item_id[]', $items, null, ['class'=>'form-control chosen-select', 'onchange' => 'setItemValue(this);']) !!}
                </td>
                <td class="text-right">
                    ৳0.00
                    {!! Form::hidden('avg_price[]', 0, ['class' => 'form-control']) !!}
                </td>
                <td>
                    {!! Form::number('item_safety_stock_qty[]', 0
                    , ['class' => 'form-control']) !!}
                </td>
                <td>
                    {!! Form::number('present_stock_qty[]', 1
                    , ['class' => 'form-control']) !!}
                </td>
                <td>
                    {!! Form::number('pipeline_qty[]', 1
                    , ['class' => 'form-control']) !!}
                </td>
                <td>
                    {!! Form::number('required_qty[]', 1
                    , ['class' => 'form-control']) !!}
                </td>
                <td>
                    {!! Form::number('approved_qty[]', 1
                    , ['class' => 'form-control']) !!}
                </td>
            ...
            <tr id="add-item-row">
                <td colspan="10" align="center">
                    <a class="bold text-decoration-none" href="javascript:addItem();"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add an Item</a>
...
function setItemValue(obj)
{
    ...
    if(itemElemValue) {
        ...
        $.ajax({
            ...
            success:function(response) {
                if(response.type == 'error') {
                    ...
                    tableRow.find('td:nth-child(2)').html('৳'+ response.avg_price.toFixed(2) +'<input type="hidden" name="avg_price[]" value="'+ response.avg_price +'">');
                    // tableRow.find('input[name^=avg_price]').val(response.avg_price);
                    tableRow.find('input[name^=item_safety_stock_qty]').val(response.safety_stock_qty);
                    tableRow.find('input[name^=remarks]').val(response.remarks);
        ...
        tableRow.find('td:nth-child(2)').html('৳0.00 <input type="hidden" name="avg_price[]" value="0">');
        tableRow.find('input[name^=item_safety_stock_qty]').val(0);
        tableRow.find('input[name^=remarks]').val('');
        avgPrice = 0;
        totalValue = approvedQty * avgPrice;
        $(this).find('td:nth-child(8)').text('৳' + totalValue.toFixed(2));
...
function addItem()
{
    ...
    //     });
    // }
    tableRow = '<tr><td>{!! Form::select('item_id[]', $items, null, ['class'=>'form-control chosen-select', 'onchange' => 'setItemValue(this);']) !!}</td><td class="text-right">৳0.00{!! Form::hidden('avg_price[]', 0, ['class' => 'form-control']) !!}</td><td>{!! Form::number('item_safety_stock_qty[]', 0, ['class' => 'form-control']) !!}</td><td>{!! Form::number('present_stock_qty[]', 1, ['class' => 'form-control']) !!}</td><td>{!! Form::number('pipeline_qty[]', 1, ['class' => 'form-control']) !!}</td><td>{!! Form::number('required_qty[]', 1, ['class' => 'form-control']) !!}</td><td>{!! Form::number('approved_qty[]', 1, ['class' => 'form-control', 'onchange' => 'updatePurchaseRequisitionInfo(this);']) !!}</td><td class="text-right">৳0.00</td><td>{!! Form::text('remarks[]', null, ['class' => 'form-control', 'placeholder' => 'Enter Remarks']) !!}</td><td><a class="btn btn-xs btn-danger" onclick="javascript:removeRow(this);"><i class="fa fa-trash-o"></i></a></td></tr>';
